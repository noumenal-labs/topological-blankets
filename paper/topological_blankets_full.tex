...
\begin{itemize}
    ...
    \item Use fluctuation-based estimates for information gain from structure changes
...
\textbf{The opportunity}: Thermodynamic quantities (Fisher information, gradient fluctuations) might provide a principled way to estimate marginal likelihoods $P(o \given m)$ without explicit integration.
...
\subsubsection{Thermodynamic Estimation}

The key quantities can be estimated from dynamics:

\textbf{Fisher Information} (curvature of $F$):
\begin{equation}
I(\theta) = \E[(\grad \log p)^2] \approx \Var[\grad E] \text{ during sampling}
\end{equation}

\textbf{Effective Dimension} (how many parameters matter):
\begin{equation}
d_{\text{eff}} = \frac{\tr(I(\theta))}{\lambda_{\max}(I(\theta))}
\end{equation}
...
\subsubsection{Thermodynamic Structure Criterion}
\begin{equation}
...
\begin{enumerate}
    ...
    \item Thermodynamic criteria correctly identify basin structure
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Can thermodynamic fluctuations predict Bayesian model evidence?}
    \begin{itemize}
        \item Fluctuation-dissipation $\to$ Fisher information $\to$ model complexity?
...
\begin{itemize}
    \item \textbf{Manifold}: The space $\mathcal{X}$ with metric induced by Fisher information
...
\begin{itemize}
    ...
    \item Topological Blankets' \textit{thermodynamic complexity} (Fisher fluctuations) as alternative to AXIOM's BMR
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Curvature as Complexity}: Is Fisher information the right geometric measure of structural complexity?
...
\textbf{Our thermodynamic criterion} (Fisher complexity) approximates this:
\begin{equation}
\text{complexity}(m) \approx \frac{1}{2} \log \det I(\theta) \approx \frac{1}{2} \sum_i \log \Var[\partial E / \partial \theta_i]
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Hessian-based coupling}: Fluctuation-dissipation estimation from sampling
...
\textbf{Fisher complexity}:
\begin{equation}
\text{complexity}(m) \approx \frac{1}{2} \sum_i \log \Var[\grad_{\theta_i} E]
\end{equation}

\subsection{Implications for the Project}

\subsubsection{Theoretical Closure}

The project now has \textit{derivational grounding} in FEP physics:
\begin{itemize}
    \item Blankets aren't ad-hoc; they're ontological necessities from sparse coupling
    \item Gradient magnitude isn't heuristic; it's the flow separatrix strength
    \item Spectral detection isn't just clustering; it's the recommended method from Friston (2025)
\end{itemize}

\subsubsection{Methodological Enhancements}

\textbf{Phase 3 upgrade}: Hybrid gradient + spectral detection (see Algorithm~\ref{alg:blanket-hybrid})

\textbf{Phase 5 upgrade}: Full recursive detection (FEP hierarchy)

\textbf{Validation upgrade}: Test on the ``active soup'' simulations from Friston (2025)

\subsubsection{Positioning Statements}

For papers/presentations:
\begin{quote}
``We operationalize the FEP's physics of emergent things in energy-based models, detecting Markov blankets via gradient magnitudes and spectral methods on Hessian estimates.''
\end{quote}

\begin{quote}
``Topological Blankets provides a unifying geometric framework for RGM, AXIOM, and EBM approaches, directly extracting blanket topology from energy landscapes, grounded in the Free Energy Principle.''
\end{quote}
...
\begin{enumerate}
    ...
    \item \textbf{Thermodynamic diagnostics}: Fisher information from fluctuations estimates complexity
...
\begin{itemize}
    ...
    \item \textbf{Thermodynamic BMR}: Use Fisher fluctuations as alternative to AXIOM's BMR criteria
\end{itemize}
...
\text{Medium } (\tau_\theta): \quad & d\theta = -\Gamma_\theta \grad_\theta F \, dt \quad \text{(natural gradient via fluctuations)} \\
...
\begin{itemize}
    ...
    \item Natural gradient via Fisher from fluctuations
...
\textbf{Thermodynamic complexity estimation} (avoiding explicit marginalization):
\begin{equation}
\text{complexity}(m) \approx \frac{1}{2} \log \det I(\theta) \approx \frac{1}{2} \sum_i \log \Var[\partial E / \partial \theta_i]
\end{equation}
...
\subsubsection{Thermodynamic Alternative to BMR}

Instead of comparing component marginal likelihoods, we compare free energies using Fisher information as a complexity measure.

\begin{algorithm}[H]
\caption{Thermodynamic Model Reduction}
\label{alg:thermo-bmr}
\begin{algorithmic}[1]
\Require Features $\mathcal{F}$ (including gradient variance), object assignments $\pi$, regularization $\lambda$
\Ensure Pruned object assignments
\For{each object $k = 1$ to $K$}
    \State $B_k \gets$ blanket variables for object $k$
    \State $\Omega_k \gets |B_k| \cdot \text{mean}(\mathcal{F}.\text{grad\_variance}[B_k])$ \Comment{Complexity}
...
\begin{enumerate}
    ...
    \item \textbf{Info-thermodynamic selection}: Fluctuation-based complexity without discrete search
\end{enumerate}
...
\subsection{From Static Geometry to Stochastic Dynamics}
...
\subsubsection{The Dynamical Perspective}
...
\subsubsection{Spectral Characterization of Metastability}
...
\subsubsection{Blankets as Information Flow Bottlenecks}
...
\subsubsection{Relating Geometry and Dynamics}
...
\subsubsection{Path-Based Markov Blankets}
...
\subsubsection{Maximum Caliber and Ontological Potential Functions}
...
\subsubsection{Dynamic Markov Blanket Detection}
...
\textbf{Connection to Topological Blankets.} The DMBD algorithm and Topological Blankets address complementary problems:
\begin{itemize}
    \item \textbf{Topological Blankets}: Given an energy $E(x)$, extract blanket structure from geometric features (gradients, Hessian)
    \item \textbf{DMBD}: Given time-series observations, infer dynamic blanket assignments and macroscopic laws
\end{itemize}

When an EBM is trained on sequential data, Topological Blankets can identify the \textit{static} blanket structure (which variables are typically at high-gradient ridges), while DMBD can reveal how blanket assignments \textit{evolve} during trajectories. The path-based formulation unifies both: objects are metastable regions with characteristic blanket statistics, identifiable either from energy geometry or path statistics.

\subsubsection{Example: Flames and Traveling Waves}
...
H_{\text{est}} &= \Cov(g) \quad \text{(fluctuation-dissipation)} \\
...
\State $H \gets \Cov(G^\top)$ \Comment{Hessian estimate via fluctuation-dissipation}
