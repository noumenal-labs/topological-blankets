...
4 & FetchPush 25D ensemble (5 members) & 0.295 & 0.500 & Task-relevant coupling; TB-guided planner=100\% \\
\midrule
...
\subsection{Continuous Manipulation: FetchPush 25D Ensemble}
\label{sec:fetchpush}

The preceding applications operate on a single agent with 8D state or 64D latent space. To test whether TB generalizes to multi-object manipulation with continuous actions, we apply it to a Bayes ensemble world model trained on the Gymnasium Robotics FetchPush-v4 task. The ensemble comprises 5 independent JAX/Equinox MLPs, each predicting state deltas $(\Delta \text{obs}, \Delta \text{ag})$ from a 25-dimensional observation vector, 3D achieved goal, and 4D continuous action. The 25D observation decomposes into gripper position (3), object position (3), relative position (3), gripper state (2), object rotation (3), object linear velocity (3), object angular velocity (3), and gripper velocity (2), with a known ground-truth partition: Object~0 = gripper, Object~1 = manipulated object, Blanket = relative position.

\subsubsection{Procedure}

Jacobian sensitivity profiles are computed for each ensemble member via automatic differentiation:
\begin{equation}
S_{ij} = \left\| \frac{\partial [\Delta\text{obs}, \Delta\text{ag}]}{\partial \text{obs}_j} \right\|_2
\end{equation}
where column norms of the Jacobian $\partial(\text{prediction})/\partial(\text{observation})$ provide per-variable sensitivity. Stacking all ensemble members yields an $(E \times N) \times 25$ gradient sample matrix passed to the TB pipeline.

\subsubsection{Results}

TB analysis on the FetchPush ensemble discovers meaningful variable groupings with best ARI = 0.295 (sensitivity-coupling method, 2 objects) and blanket F1 = 0.500 (Fisher coupling method). Multiple analysis methods discover genuine structure (positive ARI), though the learned coupling reflects task-relevant dynamics (push direction) rather than pure entity boundaries. This is expected: the ensemble's dynamics model emphasizes the causal relationships most relevant to task completion rather than the static physical decomposition of the scene.

\subsubsection{TB-Guided Task Decomposition}

The most significant finding concerns hierarchical planning. Given a \texttt{TBPartition} assigning observation variables to objects and blanket, a task-agnostic planner infers causal ordering from coupling strengths: the object with higher outgoing coupling to blanket variables is the upstream actuator (the gripper). For a two-object system, this produces a two-phase plan (approach, then push) identical to the handcrafted symbolic planner, without any task-specific knowledge. For a single-object system (FetchReach), it produces a single-phase reach plan automatically. In ablation across 50 simulated episodes, both the hardcoded and TB-guided planners achieve 100\% success, while a flat (undecomposed) CEM planner achieves only 20\%.

This result closes a loop: TB discovers structure in the learned world model, and that structure enables hierarchical planning that matches hand-engineered task decomposition. The decomposition is a consequence of the gradient geometry, not a human prior.

\paragraph{Baseline context.} The strongest existing baseline for FetchPush is DDPG with Hindsight Experience Replay \citep{andrychowicz2017her}. The official OpenAI results show a median success rate approaching 1.0 within approximately 50 epochs (5--8 million environment steps across 19 MPI workers). Notably, DDPG \textit{without} HER fails completely on FetchPush with sparse reward, underscoring the difficulty of the task. Our ensemble planner is not directly comparable in sample efficiency (different architecture, model-based vs.\ model-free), but the DDPG+HER results establish the performance ceiling that any FetchPush method should be measured against.

\paragraph{Environment version note.} The OpenAI baselines were run on FetchPush-v1; the current codebase uses FetchPush-v4. The task definition (observation space, action space, reward function, MuJoCo XML model) is identical across versions. Version changes: v1$\to$v2 migrated from \texttt{mujoco\_py} to DeepMind's \texttt{mujoco} bindings; v3 fixed a bug where \texttt{env.reset()} did not properly reset internal state; v4 fixed initial state to match the documented specification. Results are qualitatively comparable, but the reset bug means v1 initial-state distributions may differ slightly from v4. For rigorous comparison, the DDPG+HER baseline should ideally be re-run on v4, or at minimum the version difference should be noted.

\subsubsection{Uncertainty-Driven Human-Robot Collaboration}

The FetchPush ensemble also demonstrates the teleoperation story outlined in Section~\ref{sec:product-connection}. An ensemble disagreement bridge converts epistemic uncertainty into a severity signal (weighted combination of ensemble disagreement, CEM plan spread, and planner stall detection), with configurable thresholds for advisory (yellow) and handover (red) regimes. When a perturbation displaces the object to an unfamiliar configuration, ensemble members disagree on the gripper's future trajectory; this disagreement fires the catastrophe signal, triggering human takeover. Imagined trajectories (per-member rollouts of the CEM plan) visualize the prediction divergence, making uncertainty intuitive for the operator. After the human injects corrective subgoals, the agent resumes and completes the task. In the end-to-end demo, all six acceptance criteria pass: all phases covered, catastrophe fires within 5 steps of perturbation, at least 2 human goal injections, task completes (goal distance $< 0.15$), and output artifacts generated.
...
\begin{enumerate}
    ...
    \item FetchPush manipulation demo (25D, continuous actions): TB-discovered variable partition enables a task-agnostic planner that infers the approach-then-push decomposition from coupling structure alone, matching handcrafted planner performance (100\% success vs 20\% for undecomposed CEM). Ensemble disagreement drives uncertainty-aware human-robot collaboration with catastrophe signaling, imagined trajectory visualization, and scripted teleoperation
...
\begin{enumerate}
    ...
    \item Structure-aware teleoperation: use TB-discovered blanket boundaries to route human attention to decision boundaries where the world model is most uncertain, transforming teleoperation from continuous demonstration to targeted guidance at structurally meaningful intervention points. \textit{Partially demonstrated}: the FetchPush demo (Section~\ref{sec:fetchpush}) implements this pipeline end-to-end with ensemble disagreement triggering human handover, imagined trajectories visualizing prediction divergence, and human-injected subgoals resolving uncertainty. Remaining work includes integration with physical teleop hardware and validation on longer-horizon tasks
    ...
    \item Integration with hierarchical planning frameworks: use recursive blanket decomposition to generate planning abstractions automatically, with high-level plans operating over inter-object transitions and low-level controllers handling intra-object dynamics. \textit{Partially demonstrated}: the TB-guided planner (Section~\ref{sec:fetchpush}) infers causal ordering from coupling strengths and produces task decompositions matching handcrafted planners on FetchPush (approach, then push) and FetchReach (single-phase reach). Scaling to longer-horizon tasks with deeper hierarchies remains future work
\end{enumerate}
