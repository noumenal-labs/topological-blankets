...
TB_PACKAGE_DIR = os.path.expanduser("~/tb_package")
PANDAS_DIR = os.path.expanduser("~/pandas")
...
def analyze_checkpoint(checkpoint_dir, iteration, obs_data):
    ...
    try:
        ...
        obs_d = meta['obs_dim']
        act_d = meta['action_dim']
        ag_d = meta['achieved_goal_dim']
        dummy_norm = Normalizer(
            obs_mean=jnp.zeros(obs_d), obs_std=jnp.ones(obs_d),
            achieved_goal_mean=jnp.zeros(ag_d), achieved_goal_std=jnp.ones(ag_d),
            action_mean=jnp.zeros(act_d), action_std=jnp.ones(act_d),
            delta_obs_mean=jnp.zeros(obs_d), delta_obs_std=jnp.ones(obs_d),
            delta_ag_mean=jnp.zeros(ag_d), delta_ag_std=jnp.ones(ag_d),
        )
        model_template = make_model(obs_d, act_d, ag_d, config, dummy_norm, key)
        ensemble = eqx.tree_deserialise_leaves(model_path, model_template)

        # Collect Jacobians using current model API
        ...
        for obs_vec in obs_data[:n_samples]:
            action_vec = np.random.randn(act_d).astype(np.float32) * 0.1
            ag_vec = obs_vec[3:3+ag_d].astype(np.float32)
            ...
            def predict_fn(obs_part):
                ...
                for member in ensemble.members:
                    d_obs, _ = member(
                        obs_part,
                        jnp.array(ag_vec),
                        jnp.array(action_vec),
                        ensemble.normalizer,
                    )
                    delta_obs_list.append(d_obs)
                return jnp.stack(delta_obs_list).mean(axis=0)

            jac = jax.jacobian(predict_fn)(jnp.array(obs_vec))
            grads.append(np.array(jac).flatten()[:obs_d])
        ...
        for idx in GROUND_TRUTH_BLANKET:
            if idx < len(gt_blanket):
                gt_blanket[idx] = 1
        try:
            blanket_f1 = f1_score(gt_blanket, blankets.astype(int), average='binary', zero_division=0.0)
        except ValueError:
            blanket_f1 = 0.0
...
def main():
    ...
    parser.add_argument("--skip-training", action="store_true",
                        help="Skip Phase 1 (training) and run only Phases 2-4")
    ...
    if not args.skip_training:
        ...
        print("Skipping Phase 1 (--skip-training). Using existing checkpoints.")
        elapsed_total = 0
        training_log = []
        ckpts = sorted((checkpoints_dir).glob("iter_*"))
    ...
    import gymnasium_robotics; gymnasium_robotics.register_robotics_envs()
