...
def langevin_sampling(Theta: np.ndarray,
    ...
    for i in range(n_samples * n_steps):
        ...
        if i % n_steps == 0:
            ...
            gradients.append(gradient(x, Theta).copy())
...
def detect_blankets_otsu(features: Dict) -> Tuple[np.ndarray, float]:
    ...
    Otsu separates variables into two groups by gradient magnitude.
    The blanket is the *minority* group (fewer mediating variables than
    internal variables). This handles both cases:
    - High-gradient blankets (inter-basin ridges in multi-modal landscapes)
    - Low-gradient blankets (weak-coupling mediators in block-structured models)

    Known limitations:
    - Fails when blanket vars outnumber internal vars (ratio > 1.0)
    - Degrades with asymmetric object sizes (different gradient scales)
    - May miss 1-2 blanket vars when blanket is very small (< 5 vars)
    """
    ...
    # Blanket = minority group (fewer blanket vars than internal vars)
...
def dmbd_style_partition(gradients: np.ndarray,
    ...
    # Identify blanket role: lowest mean gradient magnitude.
    # In block-structured EBMs, blanket vars have weaker self-coupling
    # (smaller Hessian diagonal) and therefore lower gradient magnitude
    # than internal object variables.
    role_means = np.array([np.mean(role_features[role_labels == r, 0])
                          for r in range(n_roles)])
    blanket_role = np.argmin(role_means)
...
def axiom_style_partition(samples: np.ndarray,
    ...
    # Variables where uncertain (high-entropy) samples differ most from
    # certain samples are blanket-like, as they sit near decision boundaries.
    features = compute_geometric_features(gradients) if gradients is not None else None

    if gradients is not None and np.sum(uncertain_samples) > 0:
        # Compute per-variable variance ratio: uncertain vs certain samples.
        # Blanket variables have higher variance in uncertain samples
        # relative to certain samples (they're the dimensions that "matter"
        # for distinguishing between GMM components).
        certain_samples = ~uncertain_samples
        if np.sum(certain_samples) > 1:
            var_uncertain = np.var(gradients[uncertain_samples], axis=0) + 1e-10
            var_certain = np.var(gradients[certain_samples], axis=0) + 1e-10
            blanket_score = var_uncertain / var_certain
        else:
            blanket_score = np.var(gradients, axis=0)

        # Additionally, blanket vars have lower gradient magnitude
        # in block-structured EBMs (weaker self-coupling)
        gm = np.mean(np.abs(gradients), axis=0)

        # Combined signal: low gradient magnitude AND high variance ratio
        # Normalize both to [0,1] and average
        gm_norm = 1.0 - (gm - gm.min()) / (gm.max() - gm.min() + 1e-10)
        bs_norm = (blanket_score - blanket_score.min()) / (blanket_score.max() - blanket_score.min() + 1e-10)
        combined = 0.5 * gm_norm + 0.5 * bs_norm

        from skimage.filters import threshold_otsu
        try:
            tau = threshold_otsu(combined)
        except ValueError:
            tau = np.percentile(combined, 70)
        is_blanket = combined > tau

        # Sanity: blanket should be minority
        if np.sum(is_blanket) > len(is_blanket) // 2:
            is_blanket = ~is_blanket
    else:
        # Fallback: use low gradient magnitude
        gm = np.mean(np.abs(gradients if gradients is not None else samples), axis=0)
        from skimage.filters import threshold_otsu
        try:
            tau = threshold_otsu(gm)
        except ValueError:
...
def plot_strength_sweep(results: Dict, save_path: str = None):
    ...
    if save_path:
        ...
        save_figure(fig, 'strength_sweep', 'quadratic_toy')
...
def visualize_landscape_and_results(cfg: QuadraticEBMConfig,
    ...
    if save_path:
        ...
        save_figure(fig, 'landscape_and_results', 'quadratic_toy')
...
def run_demo():
    ...
    # Save single-example metrics
    demo_metrics = {
        'tc': tc_metrics,
        'dmbd': dmbd_metrics,
        'axiom': axiom_metrics,
    }
    demo_config = asdict(cfg)
    save_results('quadratic_toy_demo', demo_metrics, demo_config,
                 notes='Single example: 2 objects, blanket_strength=0.8')
    ...
    save_results('quadratic_toy_strength_sweep', sweep_metrics,
                 {'strengths': [0.1, 0.3, 0.5, 0.8, 1.0, 1.5, 2.0], 'n_trials': 5},
                 notes='Strength sweep from run_demo()')
